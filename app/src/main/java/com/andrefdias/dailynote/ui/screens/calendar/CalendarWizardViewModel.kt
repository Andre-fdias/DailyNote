...
class CalendarWizardViewModel @Inject constructor(
    ...
    fun regenerateEquipes() {
        ...
        val newEquipes = (0 until numEquipes).map { i ->
            ...
            // Offset days to distribute teams across the cycle
            val daysToOffset = if (turnos > 0) (i / turnos).toLong() else i.toLong()
            ...
            EquipeConfig(
                ...
                corBorda = null,
                escalaId = escalaId,
                dataInicial = LocalDate.now().plusDays(daysToOffset).toString(),
